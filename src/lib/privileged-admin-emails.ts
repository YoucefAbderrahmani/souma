const DEFAULT_PRIVILEGED_ADMIN_EMAILS_LOWER = [
...
export function isPrivilegedAdminEmail(email: string | null | undefined): boolean {
  ...
  return privilegedEmailsLower().includes(email.trim().toLowerCase());
}
