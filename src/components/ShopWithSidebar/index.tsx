...
import categoryData from "../Home/Categories/categoryData";
...
const ShopWithSidebar = ({ products }: { products: Product[] }) => {
  ...
  const categories = categoryData.map((category) => ({
    name: category.title,
    slug: category.slug,
    products: visibleProducts.filter((item) => item.category === category.slug).length,
    isRefined: selectedCategories.includes(category.slug),
  }));

  const filteredProducts = selectedCategories.length
    ? visibleProducts.filter((item) => selectedCategories.includes(item.category))
    : visibleProducts;
          ...
          <div className="flex gap-7.5">
            ...
            <div className="xl:max-w-[870px] w-full">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>
                      ...
                      <span className="text-dark">
                        {filteredProducts.length} of {visibleProducts.length}
                      </span>{" "}
                      Products
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5">
                    <button
                      ...
                      className="xl:hidden flex items-center justify-center px-3 h-9 rounded-[5px] border border-gray-3 text-dark bg-gray-1"
                    >
                      Filters
                    </button>
                    <button
              ...
              <div
                className={`${
                  productStyle === "grid"
                    ? "grid grid-cols-1 items-stretch sm:grid-cols-2 lg:grid-cols-3 gap-x-7.5 gap-y-9"
