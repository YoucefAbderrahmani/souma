...
type AddToWishlistButtonProps = {
  product: AddToWishlistProduct;
  /** Analytics `from` value (e.g. `product_grid`, `quick_view`). */
  from: string;
...
function WishlistHeartIcon({ className, filled }: { className?: string; filled?: boolean }) {
  return (
    <svg
      className={cn("fill-current transition-transform duration-300", filled && "scale-110", className)}
      width={filled ? 18 : 16}
      height={filled ? 18 : 16}
      viewBox="0 0 16 16"
      fill="none"
      ...
      aria-hidden
    >
      <path fillRule="evenodd" clipRule="evenodd" d={HEART_PATH} fill="" />
    </svg>
  );
}
...
export function AddToWishlistButton({
  ...
  const detailPrice = product.detailPrice;
  ...
  const handleClick = useCallback(() => {
    ...
    setJustToggled(true);
    window.setTimeout(() => setJustToggled(false), 520);
  ...
  }, [detailPrice, dispatch, from, inWishlist, jomlaPrice, product]);

  const iconButtonClass = cn(
    "flex items-center justify-center rounded-[5px] shadow-1 border ease-out duration-200 transition-[transform,background-color,color,border-color,box-shadow]",
    variant === "icon" ? "w-9 h-9" : "",
    inWishlist ?
      "border-red-light-3 bg-red-light-6 text-red shadow-[0_0_0_1px_rgba(242,48,48,0.12)]"
    : "border-transparent bg-white text-dark hover:border-red-light-3 hover:text-red",
    justToggled && "animate-wishlist-pop",
    "active:scale-90",
    className
  );
  ...
  if (variant === "labeled") {
    return (
      <button
        ...
        className={cn(
          "inline-flex items-center gap-2 rounded-md py-3 px-6 font-medium text-white ease-out duration-200 transition-[transform,background-color,box-shadow] active:scale-[0.98]",
          inWishlist ?
            "bg-red hover:bg-red-dark shadow-[0_4px_14px_rgba(242,48,48,0.35)]"
