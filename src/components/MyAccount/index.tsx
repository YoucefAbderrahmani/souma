...
import { updateUserData, updateUserPassword } from "./action";
...
const MyAccount = () => {
  ...
  const [userDataState, userDataAction, userDataPending] = useActionState(
    updateUserData,
    {
      error: "",
    }
  ...
  useEffect(() => {
    const currentError = (userDataState.error || passwordState.error || "") as string;
    if (currentError && currentError !== lastErrorRef.current) {
      toast.error(currentError);
      lastErrorRef.current = currentError;
      return;
    }
    if (userDataState.success || passwordState.success) {
  ...
  const UserDataChangeform = useForm<z.infer<typeof UserDataChangeSchema>>({
    resolver: zodResolver(UserDataChangeSchema),
  });
  const UserPasswordChangeform = useForm<
    z.infer<typeof UserPasswordChangeSchema>
  >({
  ...
  useEffect(() => {
    if (session?.user) {
      UserDataChangeform.reset({
        name: session.user.name ?? "",
        lastname: session.user.lastname ?? "",
        phone: session.user.phone ?? "",
      });
    }
  }, [session?.user, UserDataChangeform]);
  useEffect(() => {
    if (!session || !session.user) {
      router.replace("/");
    }
  }, [session, router]);
  function UserDataChangeonSubmit(
    values: z.infer<typeof UserDataChangeSchema>
  ) {
    ...
    startTransition(() => {
      userDataAction(formData);
    });
  }
  function UserPasswordChangeonSubmit(
            ...
            <div className="flex flex-col xl:flex-row gap-7.5">
              ...
              <div className="xl:max-w-[370px] w-full bg-white rounded-xl shadow-1">
                <div className="flex xl:flex-col">
                  <div className="hidden lg:flex flex-nowrap items-center gap-5 py-6 px-4 sm:px-7.5 xl:px-9 border-r xl:border-r-0 xl:border-b border-gray-3">
                    ...
                    <div>
                      <p className="font-medium text-dark mb-0.5 text-ellipsis">
                    ...
                    <div className="flex flex-wrap xl:flex-nowrap xl:flex-col gap-4">
                      ...
                      {shouldShowAdminNav(session.user) && (
                        <>
                          ...
                          >
                            <SellerHelperLogo size={22} variant="mono" className="hover:text-white" />
                            Seller Helper
                          </Link>
                          <Link
                            href="/admin"
                      ...
                      <button
                        ...
                        className={`flex items-center rounded-md gap-2.5 py-3 px-4.5 ease-out duration-200 hover:bg-blue hover:text-white disabled:cursor-not-allowed disabled:opacity-70 ${
                          activeTab === "logout"
                            ? "text-white bg-blue"
                            : "text-dark-2 bg-gray-1"
                        }`}
              ...
              >
                ...
                {shouldShowAdminNav(session.user) && (
                  <div className="mt-6 flex flex-wrap gap-3">
                    <Link
                      href="/seller-helper"
                      className="inline-flex items-center gap-2 rounded-md border border-gray-3 px-4 py-2 text-sm font-medium text-dark transition hover:border-[#FB923C] hover:text-[#FB923C]"
                    >
  ...
  <!-- details tab content start --> */}
              ...
              >
                <Form {...UserDataChangeform}>
                  <form
                    onSubmit={UserDataChangeform.handleSubmit(
                      UserDataChangeonSubmit,
                      (errors) => {
                    ...
                    <div className="bg-white shadow-1 rounded-xl p-4 sm:p-8.5">
                      <div className="flex flex-col lg:flex-row gap-5 sm:gap-8 mb-5">
                        <div className="w-full">
                          <FormField
                            control={UserDataChangeform.control}
                            name="name"
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="block mb-2.5 ">
                ...
                <div className="bg-white shadow-1 rounded-xl p-4 sm:p-8.5">
                  <Form {...UserPasswordChangeform}>
                    <form
                      onSubmit={UserPasswordChangeform.handleSubmit(
                        UserPasswordChangeonSubmit,
                        (errors) => {
                          console.log("Form validation errors:", errors);
                        }
                      )}
                    >
                      <div className="mb-5">
                        <FormField
                          control={UserPasswordChangeform.control}
                          name="oldPassword"
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="block mb-2.5 ">
