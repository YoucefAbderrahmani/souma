...
import { selectTotalItems, selectTotalPrice } from "@/redux/features/cart-slice";
...
const Header = () => {
  ...
  const headerIconGradientId = useId().replace(/:/g, "");

  // Get session reactively on every render
  ...
  const openDetails = (productItem: (typeof shopData)[number]) => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem("productDetails", JSON.stringify(productItem));
    }
    dispatch(updateproductDetails(productItem));
  ...
  return (
    ...
    >
      <svg aria-hidden className="pointer-events-none absolute h-0 w-0 overflow-hidden" focusable={false}>
        <defs>
          <linearGradient id={headerIconGradientId} x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#F27430" />
            <stop offset="100%" stopColor="#6677ff" />
          </linearGradient>
        </defs>
      </svg>
                    ...
                    {showSearchResults && searchQuery.trim() ? (
                      <div className="absolute left-0 top-[calc(100%+8px)] z-[10050] w-full rounded-md border border-gray-3 bg-white p-2 shadow-lg">
                        {searchResults.length > 0 ? (
                          ...
                          <p className="px-2 py-2 text-xs text-dark-4">
              ...
              <div className="flex items-center gap-5">
                ...
                >
                  <span className="inline-block relative">
                    ...
                    <span className="flex items-center justify-center font-medium text-2xs absolute -right-2 -top-2.5 bg-gradient-to-r from-blue to-[#6677ff] w-4.5 h-4.5 rounded-full text-white shadow-sm">
                      {totalItems}
                    </span>
                  </span>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-custom-sm text-[#FB923C] whitespace-nowrap">
                      {totalPrice} DA
                    </p>
                  </div>
                </button>
              ...
              <ul className="flex items-center gap-5.5">
                ...
                <li className="py-4">
                  <button
                    type="button"
                    onClick={toggleMode}
                    className={`inline-flex items-center rounded-full border px-3 py-1.5 text-custom-sm font-medium transition-all ${
                      mode === "jomla"
                        ? "border-transparent bg-gradient-to-r from-blue to-[#6677ff] text-white shadow-sm"
