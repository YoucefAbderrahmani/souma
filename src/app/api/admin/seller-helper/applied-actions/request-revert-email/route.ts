...
import { sendAppliedRecommendationRevertEmail } from "@/server/seller-helper/revert-recommendation-email";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await sendAppliedRecommendationRevertEmail(actionId);
    if (!result.ok) {
    ...
    console.error("[seller-helper/applied-actions/request-revert-email]", error);
