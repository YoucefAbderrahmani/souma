...
const Signup = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      state.error !== "" &&
      state.error !== lastErrorRef.current
    ) {
      toast.error(state.error);
      lastErrorRef.current = state.error;
    }
  }, [state.error, state.success, router, refetch]);
  ...
  function onSubmit(values: z.infer<typeof SignUpSchema>) {
    // Convert values object to FormData for formaction
    const formData = new FormData();
    Object.entries(values).forEach(([key, value]) =>
      formData.append(key, String(value))
    );

    //react neeed the formaction to be inside a startTransition thats why we wrap it
    startTransition(() => {
      formaction(formData);
    });
  }
            ...
            <div className="mt-5.5">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)}>
                  <fieldset disabled={isPending} className="disabled:opacity-95">
                    <div className="mb-5">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            ...
                            className="mb-5.5 rounded-lg border border-gray-3 bg-gray-1 placeholder:text-dark-5 w-full py-3 px-5 outline-none duration-200 focus:border-transparent focus:shadow-input focus:ring-2 focus:ring-blue/20"
                          /> */}
                          </FormControl>
                  ...
                  <button
                    type="submit"
                    className="w-full mt-7.5 flex items-center justify-center gap-2 font-medium text-white bg-dark py-3 px-6 rounded-lg ease-out duration-200 hover:bg-blue disabled:cursor-not-allowed disabled:opacity-80"
                    disabled={isPending}
                  >
                    {isPending ? (
                      <>
