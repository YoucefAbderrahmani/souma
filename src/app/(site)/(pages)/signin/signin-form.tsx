...
const Signin = () => {
  ...
  const [state, formaction, isPending] = useActionState(loginEmail, {
    error: "",
  });
  ...
  useEffect(() => {
    if (state.success) {
      router.replace("/");
      // Keep session in sync after cookie update without blocking navigation.
          ...
          <div className="max-w-[570px] w-full mx-auto rounded-xl bg-white shadow-1 p-4 sm:p-7.5 xl:p-11">
            ...
            <div>
              <form action={formaction}>
                ...
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-2 font-medium text-white bg-dark py-3 px-6 rounded-lg ease-out duration-200 hover:bg-blue mt-7.5 disabled:cursor-not-allowed disabled:opacity-80"
                  disabled={isPending}
                >
                  {isPending ? (
                    <>
